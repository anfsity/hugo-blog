...
class Search {
    ...
    private static updateQueryString(keywords: string, replaceState = false) {
        ...
        // Turbo Drive stores metadata in history.state. Overwriting it can break back/forward navigation.
        const currentState = window.history.state;
        const nextState = currentState && typeof currentState === 'object' ? { ...currentState } : currentState;

        if (replaceState) {
            window.history.replaceState(nextState, '', pageURL.toString());
        }
        else {
            window.history.pushState(nextState, '', pageURL.toString());
        }
    }
